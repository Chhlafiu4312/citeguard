...
Offline extraction is always available. DOI and arXiv metadata use fixed provider hosts when enabled. Arbitrary URL verification is disabled by default and, when enabled, rejects credentials in URLs, non-HTTP protocols, loopback names, private or reserved addresses, unsafe redirects, and responses over configured byte limits.
...
Unit tests cover DOI, arXiv, URL, Markdown, deduplication, title comparison, SSRF rejection, redirect validation, and report wording. Cordis tests cover registration and disposal. Stable CLI output has snapshots. A package check covers self-containment, types, tests, build, and archive contents.
