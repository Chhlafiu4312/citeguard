...
- `metadata` 模式只访问 Crossref 和 arXiv，普通 URL 不会被请求。
...
测试全部使用确定性假提供方，不会发起真实网络请求。`0.1.1` 是经过独立测试并发布于 [Chhlafiu4312/citeguard](https://github.com/Chhlafiu4312/citeguard) 的安全更新。包仍保持 `private: true`，不会发布到 npm。
