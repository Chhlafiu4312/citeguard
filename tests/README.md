...
A harness should mount production services and expose observable state rather than duplicate the implementation. `tests/cli.spec.ts` owns `tests/snapshots/citeguard-offline.md`. After `pnpm run prepare`, refresh it with `pnpm run snapshots:update`, then review the evidence language. Tests remain typechecked by `tsconfig.vitest.json`.
